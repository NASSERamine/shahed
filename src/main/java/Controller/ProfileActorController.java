...
public class ProfileActorController {
	...
	public static Actor act = new Actor();
    ...
    @FXML
    void switchToActorInterface(ActionEvent event) throws IOException {
    ...
    void modifier1(ActionEvent event) {
        ...
        if (nom.isEmpty() || email.isEmpty() || password.isEmpty() || dateNaissance == null) {
        	...
        	act.setDate_de_naissance(dateNaissance);
          
            
            try {
                boolean updated = actorcontroller.updateActor(act);
                if (updated) {
    ...
    public void initialize() {
    	Date dateNaissance = act.getDate_de_naissance();
        SimpleDateFormat dateFormat = new SimpleDateFormat("dd/MM/yyyy");
        String dateNaissanceStr = dateFormat.format(dateNaissance);

        datenaisstext.setText(dateNaissanceStr);

        emailtext.setText(act.getEmail());

        nametext.setText(act.getNom());
        passwordtext.setText(act.getPassword());
